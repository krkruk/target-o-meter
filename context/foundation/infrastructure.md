...
- **Storage**: Uploaded target images + the three deliverables per job live in a Railway Storage Bucket (Tigris-backed, S3-compatible), reached through django-storages' `storages.backends.s3.S3Storage`. `USE_S3=True` flips `STORAGES["default"]` to the S3 backend in prod and in docker-compose dev (MinIO); `USE_S3=False` keeps `FileSystemStorage` as the no-creds host-dev fallback. Buckets are private by default — surface deliverables via presigned URLs (`AWS_QUERYSTRING_AUTH=True` + `AWS_QUERYSTRING_EXPIRE=3600`, landed S-03 Phase 4) or backend proxying, not public bucket access (Tigris offers no public-access setting, SSE, versioning, object locks, or lifecycle rules). Create the bucket once (`railway` dashboard or `mc mb` for MinIO); the app never creates buckets at runtime. Locally, `docker-compose.dev.yml` runs MinIO + a `create-bucket` service so the S3 code path is exercised without AWS creds. **S-03 closed the S-02-deferred "S3 path raises NotImplementedError" risk**: the q2 task body now downloads the upload to a tempfile (cv2.imread cannot read an S3 key), runs the pipeline against the local bytes, then uploads the deliverables back — so `VISION_DETECTOR=google` + `USE_S3=True` against Tigris processes a real upload end-to-end.
